...
// Versão corrigida com navegação funcional
...
class FinancialAnalysisApp {
    constructor() {
        this.state = {
            ...
            produtosPorServico: [], // NOVO: Produtos consumidos por serviço
    ...
    loadInitialData() {
        ...
        // NOVO: Produtos consumidos por serviço
    ...
    loadFromStorage() {
        try {
            ...
            if (savedData) {
                ...
                // Mescla com dados iniciais, priorizando dados salvos
    ...
    switchToSection(sectionName) {
        ...
        if (targetSection) {
            ...
            // Render charts for specific sections with delay
            setTimeout(() => {
                if (sectionName === 'dashboard') {
                    this.renderDashboardCharts();
                } else if (sectionName === 'simulacoes') {
                    this.renderCharts();
    ...
    adicionarServico() {
        ...
        // Auto-criar entrada em produtos por serviço
    ...
    adicionarProduto() {
        ...
        // Auto-criar entradas em produtos por serviço para todos os serviços
    ...
    // Renderizar Produtos por Serviço (NOVO)
    ...
    calcularCustosServicos() {
        ...
        const custoFixoTotal = this.state.despesasFixas.reduce((sum, d) => sum + d.valor, 0) +
                               this.state.funcionarios.reduce((sum, f) => sum + f.salario * 1.8, 0) + // 80% encargos
                               this.state.socios.reduce((sum, s) => sum + s.proLabore, 0);
        ...
        const custoMaoObraHora = this.state.funcionarios.length > 0 ? 
            (this.state.funcionarios.reduce((sum, f) => sum + f.salario, 0) / this.state.funcionarios.length * 1.8) / horasProdutivasMes : 0;
        ...
        return this.state.servicos.map(servico => {
            ...
            const markupFactor = (100 / (100 - servico.margemDesejada)) - 1;
    ...
    renderDashboard() {
        ...
        // Calcular KPIs
        const receitaMensal = this.calculosServicos.reduce((sum, calc) => sum + calc.precoSugerido, 0) * 10; // Estimativa
    ...
    renderSimulacoes() {
        ...
        const cenarios = [0.5, 0.75, 1.0];
        const tbody = document.getElementById('cenariosTableBody');
        if (!tbody) return;
        
        tbody.innerHTML = cenarios.map(taxa => {
            const { sessoes, receita, custoVariavel, resultado } = this.calcularCenario(taxa);
            return `
                <tr>
                    <td>${taxa === 0.5 ? 'Pessimista' : taxa === 0.75 ? 'Realista' : 'Otimista'}</td>
                    <td>${(taxa * 100).toFixed(0)}%</td>
                    <td>${sessoes.toFixed(0)}</td>
                    <td>${this.formatCurrency(receita)}</td>
                    <td>${this.formatCurrency(custoVariavel)}</td>
                    <td class="${resultado >= 0 ? 'positive-value' : 'negative-value'}">
                        <strong>${this.formatCurrency(resultado)}</strong>
                    </td>
                </tr>
            `;
        }).join('');
    ...
    calcularCenario(taxaOcupacao) {
        ...
        const sessoes = horasProdutivasMes * taxaOcupacao;
        const duracaoMediaPonderada = this.calculosServicos.reduce((sum, calc) => sum + calc.servico.duracao, 0) / this.calculosServicos.length;
        const precoMedioPonderado = this.calculosServicos.reduce((sum, calc) => sum + calc.precoSugerido, 0) / this.calculosServicos.length;
        const custoVariavelMedio = this.calculosServicos.reduce((sum, calc) => sum + calc.custoMaterial + calc.custoMaoObra + calc.custoEquipamento, 0) / this.calculosServicos.length;
        
        const sessionesAjustadas = sessoes / (duracaoMediaPonderada / 60);
        const receita = sessionesAjustadas * precoMedioPonderado;
        const custoVariavel = sessionesAjustadas * custoVariavelMedio + this.state.despesasVariaveis.reduce((sum, d) => sum + d.valor, 0);
        const custoFixoTotal = this.state.despesasFixas.reduce((sum, d) => sum + d.valor, 0);
        const resultado = receita - custoVariavel - custoFixoTotal;

        return { sessoes: sessionesAjustadas, receita, custoVariavel, resultado };
    }
    ...
    renderCharts() {
        ...
        const cenarios = [
            { nome: 'Pessimista (50%)', ...this.calcularCenario(0.5) },
            { nome: 'Realista (75%)', ...this.calcularCenario(0.75) },
            { nome: 'Otimista (100%)', ...this.calcularCenario(1.0) }
        ];
        ...
        this.charts.receita = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Receita',
                    data: cenarios.map(c => c.receita),
                    backgroundColor: ['#FFC185', '#1FB8CD', '#B4413C']
                }, {
                    label: 'Resultado',
                    data: cenarios.map(c => c.resultado),
                    backgroundColor: cenarios.map(c => c.resultado >= 0 ? '#5D878F' : '#DB4545')
                }]
            },
            options: {
                responsive: true,
    ...
    renderRelatorios() {
        ...
        const cenarioBase = this.calcularCenario(this.state.parametrosAtendimento.taxaOcupacao / 100);
        
        // Indicadores
        this.updateElementText('ticketMedio', this.formatCurrency(
            this.calculosServicos.reduce((sum, calc) => sum + calc.precoSugerido, 0) / this.calculosServicos.length
        ));
        
        this.updateElementText('custoPorSessao', this.formatCurrency(
            this.calculosServicos.reduce((sum, calc) => sum + calc.custoTotal, 0) / this.calculosServicos.length
        ));
        
        const horasProdutivasMes = this.state.parametrosAtendimento.horasUteisPorDia * this.state.parametrosAtendimento.diasUteisPorMes;
        this.updateElementText('faturamentoHora', this.formatCurrency(
            cenarioBase.receita / horasProdutivasMes
        ));
        
        const margemContrib = cenarioBase.receita > 0 ? ((cenarioBase.receita - cenarioBase.custoVariavel) / cenarioBase.receita) * 100 : 0;
        this.updateElementText('margemContribuicao', margemContrib.toFixed(1) + '%');

        // DRE
        const custoFixoTotal = this.state.despesasFixas.reduce((sum, d) => sum + d.valor, 0);
        const margemContribValor = cenarioBase.receita - cenarioBase.custoVariavel;
        
        this.updateElementText('receitaBruta', this.formatCurrency(cenarioBase.receita));
        this.updateElementText('custosVariaveis', this.formatCurrency(cenarioBase.custoVariavel));
        this.updateElementText('margemContribuicaoValor', this.formatCurrency(margemContribValor));
        this.updateElementText('custosFixosTotal', this.formatCurrency(custoFixoTotal));
        this.updateElementText('resultadoLiquido', this.formatCurrency(cenarioBase.resultado));

        // Análise de Viabilidade
        this.renderAnaliseViabilidade(cenarioBase);
    }

    renderAnaliseViabilidade(cenario) {
        ...
        if (cenario.resultado > 0) {
            status = 'Negócio Viável';
            statusClass = 'status--success';
            recomendacoes.push('O negócio apresenta resultado positivo com os parâmetros atuais');
            if (cenario.resultado < 5000) {
                recomendacoes.push('Considere aumentar a taxa de ocupação ou revisar preços para melhorar a margem');
            }
        } else if (cenario.resultado > -2000) {
            status = 'Atenção Necessária';
            statusClass = 'status--warning';
            recomendacoes.push('O negócio está próximo do ponto de equilíbrio');
            recomendacoes.push('Revise custos fixos e considere estratégias para aumentar a receita');
        } else {
            status = 'Não Viável';
            statusClass = 'status--error';
            recomendacoes.push('O negócio apresenta prejuízo significativo');
            recomendacoes.push('Reavalie preços, reduza custos ou melhore a eficiência operacional');
        }
        ...
        this.charts.custos = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        custoFixoTotal,
                        this.calculosServicos.reduce((sum, calc) => sum + calc.custoMaterial, 0) / this.calculosServicos.length,
                        this.calculosServicos.reduce((sum, calc) => sum + calc.custoMaoObra, 0) / this.calculosServicos.length,
                        this.calculosServicos.reduce((sum, calc) => sum + calc.custoEquipamento, 0) / this.calculosServicos.length
                    ],
                    backgroundColor: ['#1FB8CD', '#FFC185', '#B4413C', '#5D878F']
                }]
            },
...
// Funções dos botões do dashboard
...
function imprimirRelatorio() {
    // Mover para seção de relatórios antes de imprimir
